...
import { Moon, Star, Sparkles, Cloud } from 'lucide-react';
...
export default function App() {
  ...
  return (
    /* Fundo Gradiente Noite Estrelada (Azul escuro, Índigo e Midnight) */
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 p-8 relative overflow-hidden font-sans text-slate-100">
      ...
      {/* --- ELEMENTOS DO CÉU (Van Gogh Vibes) --- */}
      <Moon className="absolute top-12 right-20 text-yellow-300 opacity-90 drop-shadow-[0_0_20px_rgba(253,224,71,0.8)]" size={120} />
      <Star className="absolute top-1/4 left-16 text-yellow-200 opacity-70 animate-pulse drop-shadow-[0_0_10px_rgba(253,224,71,0.6)]" size={45} />
      <Star className="absolute bottom-1/4 right-1/3 text-yellow-100 opacity-50 animate-pulse" size={30} style={{ animationDelay: '1s' }} />
      <Sparkles className="absolute top-1/3 right-12 text-cyan-300 opacity-40 animate-pulse" size={70} style={{ animationDelay: '0.5s' }} />
      <Cloud className="absolute bottom-10 left-10 text-blue-800 opacity-30 w-64 h-32" />
      <Cloud className="absolute top-20 left-1/3 text-indigo-900 opacity-20 w-48 h-24" />
      {/* ----------------------------------------------- */}

      {/* Container Principal (z-10 para ficar na frente do céu) */}
      <div className="max-w-3xl mx-auto relative z-10">
        
        <h1 className="text-5xl font-black mb-8 text-center tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 to-yellow-500 drop-shadow-[0_0_15px_rgba(253,224,71,0.3)]">
          GOGH Livraria 
        </h1>
